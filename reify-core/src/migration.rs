...
///
/// Uses a zero-value instance of each column to infer the SQL type.
/// Column constraints (NOT NULL, PRIMARY KEY, UNIQUE, AUTO INCREMENT) are
/// derived from the `Table::column_defs()` metadata when available; otherwise
/// all non-Option columns are emitted as `NOT NULL`.
pub fn create_table_sql<T: Table>(column_defs: &[crate::schema::ColumnDef]) -> String {
    ...
    for (i, name) in names.iter().enumerate() {
        // Find matching ColumnDef (by name)
        let def = column_defs.iter().find(|d| d.name == *name);

        // Determine SQL type from ColumnDef or fall back to TEXT
        let sql_type = if let Some(d) = def {
            if d.primary_key && d.auto_increment {
                // Use SERIAL / BIGSERIAL shorthand for auto-increment PKs
                "BIGSERIAL"
            } else if d.nullable {
                // Nullable → no NOT NULL constraint; type inferred below
                ""
            } else {
                ""
            }
        ...
        let _ = (sql_type, i); // suppress unused warnings

        // Build the column DDL
        ...
        let is_auto = def.map(|d| d.auto_increment).unwrap_or(false);
        ...
        if is_pk && is_auto {
            ...
            // We don't have a live instance to inspect, so we use a heuristic:
            // the column name suffix gives us a hint, otherwise TEXT.
            let ty = infer_sql_type_from_name(name);
            parts.push(ty.into());
        }
    ...
    let _ = pk_col; // already embedded inline
...
/// Heuristic: infer a SQL type from a column name suffix.
fn infer_sql_type_from_name(name: &str) -> &'static str {
    if name == "id" || name.ends_with("_id") {
        "BIGINT"
    } else if name.ends_with("_at") || name.ends_with("_on") {
        "TIMESTAMPTZ"
    } else if name.ends_with("_count") || name.ends_with("_num") {
        "INTEGER"
    } else if name.ends_with("_price") || name.ends_with("_amount") || name.ends_with("_rate") {
        "NUMERIC"
    } else if name.ends_with("_enabled") || name.ends_with("_active") || name.starts_with("is_") {
        "BOOLEAN"
    } else {
        "TEXT"
    }
...
pub fn add_column_sql(table: &str, column: &str, def: Option<&crate::schema::ColumnDef>) -> String {
    let is_nullable = def.map(|d| d.nullable).unwrap_or(false);
    let ty = infer_sql_type_from_name(column);
    let null_clause = if is_nullable { "" } else { " NOT NULL" };
    let default_clause = if !is_nullable {
        // Provide a safe default so existing rows don't violate NOT NULL
        format!(" DEFAULT {}", default_for_type(ty))
    } else {
        String::new()
    };
    format!("ALTER TABLE {table} ADD COLUMN {column} {ty}{null_clause}{default_clause};")
}

fn default_for_type(ty: &str) -> &'static str {
    match ty {
        "BIGINT" | "INTEGER" | "SMALLINT" | "NUMERIC" => "0",
        "BOOLEAN" => "FALSE",
...
impl MigrationRunner {
    ...
    pub fn add_table<T: Table>(mut self) -> Self {
        // We need ColumnDef metadata. Tables that implement Schema provide it;
        // for plain Table impls we synthesise minimal defs from column_names().
        let column_defs: Vec<crate::schema::ColumnDef> = T::column_names()
            .iter()
        ...
        let create_sql = create_table_sql::<T>(&column_defs);
    ...
    where
        T: Table,
    {
        let create_sql = create_table_sql::<T>(&schema.columns);
        self.tables.push(TableEntry {
            table_name: T::table_name(),
            column_names: T::column_names(),
    ...
    async fn ensure_tracking_table(&self, db: &dyn Database) -> Result<(), MigrationError> {
    ...
    async fn existing_columns(
        ...
        match rows {
            ...
            Err(_) => Ok(None), // treat errors as "table absent"
        }
    }
                ...
                if let Some(existing_cols) = existing {
                    ...
                    for col in entry.column_names {
                        if !existing_cols.iter().any(|c| c == col) {
                            ...
                            stmts.push(add_column_sql(entry.table_name, col, def));
                        }
                    }
                    if !stmts.is_empty() {
    ...
    pub fn diff<'a>(
        &'a self,
        db: &'a dyn Database,
    ) -> Pin<Box<dyn Future<Output = Result<SchemaDiff, MigrationError>> + Send + 'a>> {
        Box::pin(async move {
                        ...
                        for col_name in entry.column_names {
                            match db_cols.iter().find(|c| c.name == *col_name) {
                                ...
                                Some(db_col) => {
                                    ...
                                    // Type check: normalise the inferred struct type
                                    // so it compares fairly against the DB type.
                                    let struct_type = normalize_sql_type(
                                        infer_sql_type_from_name(col_name),
                                    );
    ...
    pub fn dry_run<'a>(
        &'a self,
        db: &'a dyn Database,
    ) -> Pin<Box<dyn Future<Output = Result<Vec<MigrationPlan>, MigrationError>> + Send + 'a>> {
        Box::pin(async move {
            self.ensure_tracking_table(db).await?;
            let applied = self.applied_versions(db).await?;
    ...
    pub fn status<'a>(
        &'a self,
        db: &'a dyn Database,
    ) -> Pin<Box<dyn Future<Output = Result<Vec<MigrationStatus>, MigrationError>> + Send + 'a>>
    {
        Box::pin(async move {
            self.ensure_tracking_table(db).await?;
            let applied = self.applied_versions(db).await?;
...
mod tests {
    ...
    fn create_table_sql_contains_all_columns() {
        ...
        let sql = create_table_sql::<Users>(&defs);
